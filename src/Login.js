...
function Login({ onSwitch, onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('https://foodreels-backend.onrender.com/login', {
        method: 'POST',
  ...
  return (
    <div className="fade-in" style={{
      ...
      <div style={{
        ...
        {message && (
          <div style={{ backgroundColor: '#2a1a1a', border: '1px solid #e85d04', borderRadius: '10px', padding: '12px', marginBottom: '16px', color: '#e85d04', fontSize: '14px', textAlign: 'center' }}>{message}</div>
        )}
        ...
        <input
          ...
          style={{ width: '100%', padding: '14px', marginBottom: '12px', borderRadius: '10px', border: '1px solid #2a2a2a', backgroundColor: '#2a2a2a', color: 'white', fontSize: '15px', boxSizing: 'border-box', outline: 'none' }}
        />
        <input
          ...
          style={{ width: '100%', padding: '14px', marginBottom: '24px', borderRadius: '10px', border: '1px solid #2a2a2a', backgroundColor: '#2a2a2a', color: 'white', fontSize: '15px', boxSizing: 'border-box', outline: 'none' }}
        />
        ...
        <p style={{ textAlign: 'center', marginTop: '20px', color: '#888', fontSize: '14px' }}>
          New here? <span onClick={onSwitch} style={{ color: '#e85d04', cursor: 'pointer', fontWeight: 'bold' }}>Create account</span>
        </p>
      </div>
    </div>
  );
